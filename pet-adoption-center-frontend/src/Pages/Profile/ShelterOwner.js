...
const ShelterOwner = () => {
  ...
  const handleAddActivitySubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${address_url}/Shelters/${shelterId}/calendar/activities`,
        {
  ...
  function deletePetAdoption(
    shelterId,
    adoptionId,
    adoptionPetId,
    adoptionUserId
  ) {
    const url = `${address_url}/Shelters/${shelterId}/adoptions/${adoptionId}/pets/${adoptionPetId}/users/${adoptionUserId}`;
    ...
    axios
      ...
      .then((response) => {
        if (response.status === 204) {
          ...
          setShelterAdoptions((currentAdoptions) =>
            currentAdoptions.filter((adoption) => adoption.Id !== adoptionId)
          );
        }
      })
      .catch((error) => {
        if (error.response) {
          if (error.response.status === 404) {
            console.error('Adoption not found');
          } else {
  ...
  return (
    <div className="shelter-owner-container">
            ...
            <ul className="shelter-pets-list">
              {petsAvailable.map((pet, index) => (
                <p>
                  ...
                  <button
                    className="buttonAddMedicalHistory"
                    onClick={() => {
                  ...
                  <Modal
                    isOpen={diseaseVisible}
                    onRequestClose={() => setDiseaseVisible(false)}
                  >
                    <div className="modal-shelter-pets-add-vacination">
                        ...
                        <label>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setDiseaseForm({
                                ...diseaseForm,
                                illnessStart: e.target.value + ':00.000Z',
                              })
                            }
                          />
                        </label>
                        <label>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              setDiseaseForm({
                                ...diseaseForm,
                                illnessEnd: e.target.value + ':00.000Z',
                              })
                            }
                          />
                        </label>
                      ...
                      <button
                        onClick={() => handleAddDisease(shelterId, pet.Id)}
                        className="addDisease"
                      >
                        Add Disease
                      </button>
                      <button
                        className="buttonDiseaseBack"
                        onClick={() => setDiseaseVisible(false)}
                      >
                        Back
                      </button>
                    </div>
                  ...
                  <button
                    className="buttonAddVacination"
                    onClick={() => {
                      setVacinationVisible(true);
                    }}
                  >
                    Add vacination
                  </button>
                  <Modal
                    isOpen={vacinationVisible}
                    onRequestClose={() => setVacinationVisible(false)}
                  >
                    <div className="modal-shelter-pets-add-vacination">
                      <form>
                        <label>
                      ...
                      <button
                        onClick={() => handleAddVacination(shelterId, pet.Id)}
                        className="addVaccination"
                      >
                        Add Vaccination
                      </button>
                      <button
                        className="buttonVacinationBack"
                        onClick={() => setVacinationVisible(false)}
                      >
                        Back
                      </button>
                    </div>
            ...
            <ul className="shelter-adoptions-list">
              {shelterAdoptions.map((adoption) => (
                <div className="adoption-info" key={adoption.Id}>
                  <p>
                    <button
                      className="shelter-adoption-delete-button"
                      onClick={() =>
                        deletePetAdoption(
                          shelterId,
                          adoption.Id,
      ...
      <div className="shelter-calendar-activites-shelter-owner">
        ...
        <button
          className="buttonAddActivity"
          onClick={() => {
            setActivityAddVisible(true);
          }}
        >
          Add Activity
        </button>
        <Modal
          isOpen={activityAddVisible}
          onRequestClose={() => setActivityAddVisible(false)}
        >
            ...
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setActivityFormData({
                    ...activityFormData,
                    startActivityDate: e.target.value + ':00.000Z',
                  })
                }
              />
            </label>
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setActivityFormData({
                    ...activityFormData,
                    endActivityDate: e.target.value + ':00.000Z',
                  })
                }
              />
            </label>
          ...
          <button
            className="button-add-activity-shelterowner"
            onClick={handleAddActivitySubmit}
          >
            Add Activity
          </button>
          <button
            className="buttonShelterActivityBack"
            onClick={() => setActivityAddVisible(false)}
          >
            Back
          </button>
        </Modal>
