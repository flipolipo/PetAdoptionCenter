...
const PreadoptionPoll = ({ shelterId, petId, userId }) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <select name="over18" onChange={handleChange} value={formData.over18}>
      ...
      <label>Do ALL the members of your household want a new pet?*</label>
      <select
      ...
      {formErrors.householdMembersWantPet && (
        <p className="error-message">{formErrors.householdMembersWantPet}</p>
      )}
      <label>Have you relinquished or given away any pets before?*</label>
      <select
      ...
      {formErrors.relinquishedPetsBefore && (
        <p className="error-message">{formErrors.relinquishedPetsBefore}</p>
      )}
      <label>
        Please list current pets residing at your home (including roommates'
        pets also). Include pets' breed, age, sex, spay/neuter status, number of
        years owned, and if they live indoors or outdoors. *
      </label>
      <input
        type="text"
      ...
      <label>
        In a 24-hour day, how long would the pet be left alone at a given time?*{' '}
      </label>
      <input
        type="number"
      ...
      <label>Where will your new pet be kept when you are home?* </label>
      <input
        type="text"
      ...
      {formErrors.petLocationWhenHome && (
        <p className="error-message">{formErrors.petLocationWhenHome}</p>
      )}
      <label>Where will your new pet be kept when you are NOT at home?* </label>
      <input
        type="text"
      ...
      {formErrors.petLocationWhenNotHome && (
        <p className="error-message">{formErrors.petLocationWhenNotHome}</p>
      )}
      <label>In a 24-hour day, how long would the animal be indoors?* </label>
      <input
        type="number"
        name="petIndoorsTime"
      ...
      <label>Do you already have a veterinarian? If yes, Name:* </label>
      <input
        type="text"
